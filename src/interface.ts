  ...
  document.querySelector<HTMLDivElement>("#app")!.innerHTML = `
    ...
    <div id="hud">
      ...
      <aside class="camp-panel" id="camp-panel"><div class="panel-heading"><div><p class="eyebrow">MAKE YOURSELF AT HOME</p><h2>나의 개척지 <span id="camp-level">Lv.1</span></h2></div><span class="camp-seal">${icon("camp")}</span></div>
        ...
        <div id="pane-automation" class="tab-pane" role="tabpanel" aria-labelledby="tab-automation" hidden><p class="section-caption">함께하면 숲은 더 넓어집니다 <span id="slot-count">0 / 6</span></p>${["hire", "tower", "carrier", "anchor"].map((id) => card(`${id}-btn`)).join("")}</div>
